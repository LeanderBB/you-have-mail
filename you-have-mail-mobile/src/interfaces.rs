...
pub trait Notifier: Send + Sync {
    ...
    fn account_added(&self, email: String, backend: String, proxy: Option<Proxy>);
...
impl you_have_mail_common::Notifier for NotifierWrapper {
    fn notify(&self, notification: you_have_mail_common::Notification) {
        ...
        match notification {
            ...
            Not::AccountAdded(email, backend, proxy) => {
                self.0
                    .account_added(email.to_string(), backend.to_string(), proxy.cloned())
            }
            ...
            Not::ConfigError(e) => self.0.error(e.to_string()),
        }
    }
}
